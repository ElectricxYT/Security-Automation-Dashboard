...
const cors = require("cors"); //Allows frontend to access backend without CORS issues
app.use(cors());
 ...
 */
app.get("/verdict/:indicator", async (req, res) => {
  try {
    const target = req.params.indicator;
    ...
    //
    const vtNormalized = RISK_SCALE[vtResult.risk] ?? 0.6;
    const abuseNormalized = RISK_SCALE[abuseResult.risk] ?? 0.6;

    const finalNormalized = Math.max(vtNormalized, abuseNormalized);
    ...
    // Convert normalized score back to label for front-end display
    function normalizedToLabel(score) {
    ...
    const finalRiskFrontEnd = normalizedToLabel(finalNormalized);
    ...
    res.json({
      ...
      sources: {
        virustotal: {
          ...vtResult,
          normalizedScore: vtNormalized
        },
        abuseipdb: {
          ...abuseResult,
          normalizedScore: abuseNormalized
        }
      }
    });
